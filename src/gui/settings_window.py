...
class SettingsWindow(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.chk_bool_ops = QCheckBox("Enable boolean operators (AND/OR/NOT)")
        self.chk_topic_hierarchy = QCheckBox("Enable topic hierarchy")
        ...
        self.spin_topic_depth = QSpinBox(self.tab_search)
        self.spin_topic_depth.setRange(1, 10)
        fs.addRow(self.chk_semantic)
        fs.addRow(self.chk_fallback)
        fs.addRow(self.chk_autocomplete)
        fs.addRow(self.chk_bool_ops)
        fs.addRow(self.chk_topic_hierarchy)
    ...
    def _apply_config(self, cfg: ApplicationConfig) -> None:
        ...
        self.chk_autocomplete.setChecked(bool(ss.enable_auto_complete))
        self.chk_bool_ops.setChecked(bool(ss.enable_boolean_operators))
        self.chk_topic_hierarchy.setChecked(bool(ss.enable_topic_hierarchy))
        ...
        self.spin_topic_depth.setValue(int(ss.topic_hierarchy_depth))
        # Perf
    ...
    def _collect_config(self) -> ApplicationConfig:
        ...
        ss.enable_auto_complete = self.chk_autocomplete.isChecked()
        ss.enable_boolean_operators = self.chk_bool_ops.isChecked()
        ss.enable_topic_hierarchy = self.chk_topic_hierarchy.isChecked()
        ss.topic_hierarchy_depth = int(self.spin_topic_depth.value())
